...
Much of the work comes from the literature of Will G. Hopkins, but several other studies by researchers around the world are also credited with the statistical concepts featured throughout this package.
...
Second, psr::ICC_long() allows for the simultaneous computation of the ICC for multiple metrics, however many the user wants, whereas psych::ICC() is limited in that, as its input data must be in wide form, it can only calculate the ICC for one metric at a time. This is due to the fact that data in "wide" form by construction include only the measurements of one metric.
...
In the following error example, the trials are all labeled correctly, but the first metric is entered as a character vector, when it needs to be a numeric vector, and the :
