...
There are other interpretations of the SWC in the literature. For example, Paton & Hopkins (2005) utilize the typical percentage error (the typical error as a percentage of a mean) instead of the between-athlete standard deviation, and multiply this number by 0.5 to obtain the SWC in percentage terms. Other studies also use the TE in its calculation of the SWC, but multiply it by 1.65 (or 1.96) * the square root of 2, which represents a substantially larger effect size than the one we use (Mann et al., 2016; Mann et al., 2014). This package uses the interpretation of the statistic found in Bernards et al. (2017), as it represents a way of examining change that is distinct from the MDC, which is also included in this package. This approach is found in other studies as well, including Swinton et al. (2018). 
